...
// for the Dialed Network redesign. Cool blue palette + DM Sans.

const { useState, useRef, useEffect, useCallback } = React;

// ── tokens ────────────────────────────────────────────────────
const C = {
  bg: '#EEF1FA', surface: '#FFFFFF', elevated: '#F4F7FF', border: '#DDE3F0',
  hair: 'rgba(0,0,0,0.07)', ink: '#0D1526', ink2: '#5C6B8A', ink3: '#95A2BC',
  accent: '#2563EB', overdue: '#EF4444', warning: '#F59E0B', success: '#16A34A',
  inkCard: '#0D1526',
};
...
function Card({ children, style, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      boxShadow: '0 4px 20px rgba(13,21,38,0.05)', ...style,
    }}>{children}</div>
  );
}
