...
function makeStyles(c: ColorPalette) {
  return StyleSheet.create({
    ...
    suggestBtn: {
      backgroundColor: c.primary, borderRadius: Radius.full, paddingVertical: 13,
      alignItems: 'center',
    },
    suggestBtnText: { fontFamily: FontFamily.sansMedium, fontSize: 15, color: c.surface },
    skeletonBox: { gap: 10, paddingTop: 12 },
    venueCard: {
    ...
    venueIconWrap: {
      ...
      backgroundColor: `${c.gold}12`, alignItems: 'center', justifyContent: 'center',
    },
    ...
    vibePill: {
      ...
      borderRadius: Radius.full, backgroundColor: `${c.gold}15`, marginTop: 6,
    },
    vibeText: { fontFamily: FontFamily.sansMedium, fontSize: 10, color: c.gold },
    ...
    stepNumText: { fontFamily: FontFamily.sansMedium, fontSize: 13, color: c.gold },
    ...
    agendaDot: { width: 5, height: 5, borderRadius: 2.5, backgroundColor: c.gold, marginTop: 7, flexShrink: 0 },
...
export default function CoffeeDatesPage() {
  ...
  async function suggestVenues() {
    ...
    try {
      const res = await fetch(`${getApiUrl()}/api/content/venue-suggestions`, {
        method: 'POST',
      ...
      refreshProgress()
      AsyncStorage.setItem(cacheKey, JSON.stringify({ data: suggestions, ts: Date.now() })).catch(() => {})
    } catch {
      setVenues([])
    } finally {
  ...
  return (
    <View style={styles.screen}>
      <TopicPageHeader title="Coffee Dates" completedCount={completedCount} totalItems={TOTAL_ITEMS} />
          ...
          {venues.map((v, i) => (
            <View key={i} style={styles.venueCard}>
              <View style={styles.venueIconWrap}>
                <Ionicons name="cafe-outline" size={20} color={c.gold} />
              </View>
              <View style={{ flex: 1 }}>
          ...
          {STEPS.map((s, i) => (
            <View key={i}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.75}
              >
                <View style={[styles.stepRow, { marginBottom: 0, flex: 1 }]}>
                  <View style={styles.stepNum}><Text style={styles.stepNumText}>{s.num}</Text></View>
                  <Text style={[styles.stepTitle, { flex: 1 }]}>{s.title}</Text>
                </View>
                <Ionicons name={expandedStep === i ? 'chevron-up' : 'chevron-down'} size={16} color={c.tertiary} />
              </TouchableOpacity>
              {expandedStep === i && (
                <Text style={[styles.stepBody, { marginTop: 8, marginLeft: 40 }]}>{s.body}</Text>
              )}
            </View>
          ))}
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.tabRow}>
            {(['30 min', '45 min', '60 min'] as const).map(tab => (
              <TouchableOpacity
                key={tab}
                style={[styles.tab, agendaTab === tab && styles.tabActive]}
                onPress={() => setAgendaTab(tab)}
                activeOpacity={0.7}
              >
